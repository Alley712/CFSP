...
| `chars` | 中文标点池 | 插入的标点候选 |
| **Epoch** | **不削减** | 保持原 epoch：Task1=10, Task2=5, Task3=10 |

> **Epoch 决策演变**：最初计划 epoch 减半以控制训练时间，但消融实验揭示了真实原因：
>
...
> | E9 | 71.63 | 5 | 有 | -0.37 |
>
> - **epoch 减半是主因**：仅砍 epoch 就导致 -2.29
> - **AEDA 本身有价值**：同样 5 epoch 下，AEDA 回收了 +1.92 分（69.71 → 71.63），回收率 84%
> - **结论**：恢复全 epoch（10/5/10），训练时间 ~10.6h
...
| 训练集翻倍 → 训练时间翻倍 | 消融证实 epoch 减半有害（-2.29），改为全 epoch（~10.6h）|
